function [velo,rmse] = lidar_merge_cpd(velo_c,velo_l)
    %将上一帧（l）的数据叠加到当前帧（c）
    ...
    while (rmse>3 && test_count<5) % 误差精度小于2 停止
        ...
        %moving = pcdownsample(pc2, 'gridAverage', gridSize);
        fixed = pcdownsample(pc1, 'random', 0.1);
        moving = pcdownsample(pc2, 'random', 0.1);
        
        %fixed=pc1;moving=pc2;
        [tform,movingReg,rmse] = pcregistercpd(moving, fixed, 'Transform','Rigid','OutlierRatio',0.1, 'MaxIterations',20);
