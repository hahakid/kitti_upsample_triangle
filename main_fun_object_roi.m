...
save_path='H:\kitti\object\training\LidarROI';
...
for frame = fst_frame: 1: nt_frames
    ...
    %scatter(velo_c(:,2),-velo_c(:,1),1,'red');
    
    %if ~exist(sprintf('%s/%d.bin',save_path,frame))
    %    mkdir(sprintf('%s/%d.bin',save_path,frame));
    %end
    fd=fopen(sprintf('%s/%06d.bin',save_path,frame),'wb');
