function [velo,rmse] = lidar_merge_ndt(velo_c,velo_l)
    %将上一帧（l）的数据叠加到当前帧（c）
    ...
    test_count=0;
    while (rmse>3 && test_count<3) % 误差精度小于2 停止
        ...
        gridStep=0.5;
        %[tform,movingReg,rmse] = pcregisterndt(moving, fixed, gridStep,'OutlierRatio',0.55,'MaxIterations',10);
