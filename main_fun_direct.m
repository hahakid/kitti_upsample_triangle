...
fst_frame = 1; nt_frames = nimages-1;% for icp 叠加，从第二帧开始，用上一帧叠加当前帧
...
for frame = fst_frame: 1: nt_frames
    ...
    bt=cputime;
    [velo,rmse_direct,rmse]=lidar_merge_direct(velo_c,velo_l);
    ...
    up_im=Fun_upsample(pixel,img,st,interp_method);%上采样+稠密化
    time=cputime-bt;
    %imshow(up_im);
    imwrite(up_im,sprintf('%s%06d.png',c_save_dir,frame));%保存表示深度信息的灰度图
    %信息输出
    point_num=length(pixel(:,1));
    info_str=sprintf('%06d %d %fs %0.5f %0.5f',frame,point_num,time,rmse_direct,rmse);% 没有rmse
    writelabels(save_path,seq_idx,info_str,'direct');
end
end
